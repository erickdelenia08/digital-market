...
type OrderStatus = 'PAID' | 'PENDING' | 'EXPIRED' | 'FAILED';
...
interface Order {
  ...
  paymentUrl?: string; // Untuk melanjutkan pembayaran jika PENDING
  items: OrderItem[];
}

// Dummy data transaksi yang merefleksikan Prisma Schema Order & OrderItem
const orders: Order[] = [
  {
    id: 'ord-101',
    orderNumber: 'ORD-2026-8891',
    createdAt: '22 Jul 2026, 10:15 WIB',
    totalAmount: 250000,
    status: 'PAID',
    paymentMethod: 'QRIS',
    items: [
      {
        id: 'item-1',
        productName: 'Modern Tactile UI Kit & Component Library',
        productSlug: 'modern-tactile-ui-kit',
        price: 250000,
      },
    ],
  },
  {
    id: 'ord-102',
    orderNumber: 'ORD-2026-8840',
    createdAt: '20 Jul 2026, 14:30 WIB',
    totalAmount: 180000,
    status: 'PENDING',
    paymentMethod: 'BCA Virtual Account',
    paymentUrl: 'https://payment-gateway-link.com/pay',
    items: [
      {
        id: 'item-2',
        productName: 'Next.js Digital Marketplace Starter Boilerplate',
        productSlug: 'nextjs-marketplace-boilerplate',
        price: 180000,
      },
    ],
  },
  {
    id: 'ord-103',
    orderNumber: 'ORD-2026-7201',
    createdAt: '15 Jun 2026, 09:00 WIB',
    totalAmount: 99000,
    status: 'EXPIRED',
    paymentMethod: 'GoPay',
    items: [
      {
        id: 'item-3',
        productName: 'Minimalist Icon Pack (500+ Vector Icons)',
        productSlug: 'minimalist-icon-pack',
        price: 99000,
      },
    ],
  },
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-500 font-medium mt-1">
                              Metode Bayar: <span className="text-slate-700 font-semibold">{order.paymentMethod}</span>
                            </p>
                          </div>
                        </div>
                  ...
                  <div className="p-5 bg-slate-50/40 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      {order.status === 'PAID' && (
                        <>
                          <button
                            className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl border border-slate-200/80 transition-colors shadow-inner"
                          >
                            Download Receipt
                          </button>
                          <Link
                      ...
                      {order.status === 'PENDING' && (
                        <>
                          <button
                            className="px-4 py-2 bg-slate-100 hover:bg-rose-50 hover:text-rose-600 text-slate-600 text-xs font-bold rounded-xl border border-slate-200 transition-colors"
                          >
                            Batalkan
                          </button>
                          {order.paymentUrl && (
