...
type CartItems = Awaited<ReturnType<typeof getCart>>;
interface Props {
    session: Session | null;
    initialCart: CartItems
}

const CartClient = ({ session, initialCart }: Props) => {
    ...
    const [dbCart, setDbCart] = useState(initialCart);
    const cartData = isLogin
        ? dbCart
        : localCart;

    // const id = isLogin ? item.productId : item.id;
    // const product = isLogin ? item.product : item;

    const subtotal = isLogin
        ? dbCart
    ...
    const removeItem = async (id: string) => {
        // rollback
        const previousDbCart = [...dbCart];

        try {
            if (isLogin) {
                // Optimistic UI Update: hapus langsung dari tampilan
                setDbCart(prev => prev.filter(i => i.productId !== id));

                // Panggil Server Action
                const result = await removeFromCartDB(userId!, id);

                // Jika server mengembalikan status gagal
            ...
            // Hapus centangan/checklist terpilih
            setSelectedIds(prev => prev.filter(i => i !== id));
            toast.success("Produk berhasil dihapus");
        } catch (error) {
            // 🔄 ROLLBACK: Kembalikan tampilan ke kondisi semula jika gagal di server
            if (isLogin) {
                setDbCart(previousDbCart);
            }
                        ...
                        <AnimatePresence mode="popLayout">
                            {cartData.length > 0 ? (
                                cartData.map((item: any) => {
                                    const product = isLogin ? item.product : item;
                                    const itemId = isLogin ? item.productId : item.id;
